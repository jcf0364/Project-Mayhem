...
public class KnockbackReceiver : MonoBehaviour
{
    ...
    private void ApplyKnockback(HitInfo info)
    {
        ...
        //Prefer the attacker's facing - reliable even when fighters overlap.
        var attackerMove = info.Source.GetComponent<LocalPlayerMovement>();
        if (attackerMove != null)
        {
            dir = new Vector2(attackerMove.FacingDirection, 0f);
        }
        else
        {
            // Fallback: away from where the attack came from.
            dir = ((Vector2)transform.position - info.SourcePosition).normalized;
